...
struct IpCheckerMeta: MetaProviderProtocol {
    ...
    static func getFullDesc(appName: String) -> String {
        let fullDesc1 = "\(appName) is a handy and useful mobile application that allows users to quickly and easily check IP address information. This app provides many useful features for analyzing and researching IP addresses to help users get the most information about them. One of the key features of \"\(appName)\" is the ability to check an IP address for location. Users can enter an IP address and get information about what country, region, and city that IP address is located in. This can be useful for location, activity tracking, or just for curiosity"
        let fullDesc2 = "The \(appName) app also provides information about the Internet Service Provider associated with the IP address. Users can find out the name of the ISP, its contact information, and other details. This can be useful for checking network reliability and security, as well as for debugging Internet connection problems. Another important feature of \(appName) is the ability to check an IP address for blocking or spam lists. Users can find out if an IP address is blacklisted or has a bad reputation. This can help users determine if an IP address is insecure or suspicious"
        let fullDesc3 = "The \(appName) app also provides information about the type of connection associated with an IP address. Users can find out whether an IP address is or dynamic, and get more information about network settings. This can be useful for managing network settings and understanding connection patterns. \"\(appName)\" also provides the user with the ability to scan the ports associated with an IP address. Users can check which ports are open or closed on a particular IP address. This can be useful for checking network security and detecting potential vulnerabilities"
        let fullDesc4 = "The \(appName) application offers a user-friendly and intuitive interface that makes using the application simple and easy for users of all skill levels. It also provides fast, accurate IP address verification using reliable data sources. In addition, \(appName) provides the ability to store a history of IP address checks so that users can easily track their requests and access previously obtained information. This allows users to save and organize data for future use"
        let fullDesc5 = "\(appName) is a handy mobile app to help you get information about any IP address. Whether it's the IP address of your own device or the IP address of another host on the network, \(appName) will provide you with all the data and statistics you need. All in all, \"\(appName)\" is a reliable and useful application that helps users to get complete information"
        let fullDesc6 = "With \(appName), you can find out the location of an IP address. The application uses advanced geolocation technology to pinpoint the geographic location of a host. You will know the country, region, and even city associated with a given IP address. This can be useful, for example, to track the location of your devices or to check where suspicious activity is coming from"
        let fullDesc7 = "The \(appName) provides information about the type of IP address. You will know whether the IP address is or dynamic. This can be useful for understanding which devices have permanent IP addresses and which may change over time"
    ...
    static func getShortDesc(appName: String) -> String {
        let shortDesc1 = "Know your IP, protect your privacy!"
        let shortDesc2 = "\(appName): guardian of your digital identity!"
        let shortDesc3 = "Stay safe with \(appName), your personal IP tracker!"
        let shortDesc4 = "Track, check and secure your IP with \(appName)!"
        let shortDesc5 = "\(appName): uncover the secrets of your online presence!"
        let shortDesc6 = "Discover your IP secrets with \(appName)!"
        let shortDesc7 = "Keep your IP under control with \(appName), the ultimate tracker!"
        let shortDesc8 = "\(appName): giving the knowledge of your online identity!"
        let shortDesc9 = "\(appName): your indispensable tool for monitoring and protecting your IP address!"
        let shortDesc10 = "Stay informed, stay protected with \(appName)!"
