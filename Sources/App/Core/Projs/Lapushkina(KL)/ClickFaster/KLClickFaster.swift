    ...
    when (viewModel.uiState.collectAsState().value) {
        ...
        is MainUiState.Game -> {
            ...
            ScreenContent(
                ...
                textColor = textColorPrimary,
                message = time,
                clickMessage = stringResource(R.string.finish_text, clicks),
                clickAction = { clicks++ }
            )
        }
        is MainUiState.Finish -> {
            ScreenContent(
                ...
                message = stringResource(R.string.finish_text, clicks),
...
sealed class MainUiState {
    ...
    object Game : MainUiState()
    object Finish : MainUiState()
}
...
class MainViewModel @Inject constructor() : ViewModel() {
    ...
    private val _time = MutableStateFlow(TIME_START)
    val time = _time.asStateFlow()

    private val timer = object : CountDownTimer(COUNTDOWN_TIME, COUNTDOWN_INTERVAL) {
        override fun onTick(millisUntilFinished: Long) {
            _time.value = format(millisUntilFinished)
        }

        override fun onFinish() {
            _time.value = TIME_START
            _uiState.value = MainUiState.Finish
        }
    }

    fun start() {
        _uiState.value = MainUiState.Game
    ...
    private fun format(duration: Long): String {
        ...
        return SECOND_FORMAT.format(seconds, milliseconds)
    }

    companion object {
        private const val COUNTDOWN_TIME = 5000L
        private const val COUNTDOWN_INTERVAL = 10L
        private const val SECOND_FORMAT = "%02d:%02d"
